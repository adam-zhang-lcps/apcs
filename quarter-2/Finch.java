/**
 * This class extends the Robot class to incorporate functions to control the
 * inputs and outputs of the Finch. It includes methods to set the values of
 * motors and LEDs, as well as methods to read the values of the sensors.
 *
...
public class Finch extends Robot {
    ...
    public Finch(String device) {
        if (!((device == "A") || (device == "B") || (device == "C"))) {
            ...
            // The finch has separate requests for these so that the results
            // returned
    ...
    /**
     * This function sends a request to BlueBird Connector to determine whether
     * or not the device is a Finch.
     */
    private boolean isFinch() {
        StringBuilder newURL = new StringBuilder(baseUrl);
        String testURL =
            (newURL.append("in/isFinch/static/").append(deviceInstance))
                .toString();
     ...
     */
    private void moveFinchAndWait(String motion, String direction,
        ...
        while (!(
            (System.currentTimeMillis() > commandSendTime + 500 || wasMoving) &&
            !isMoving)) {
            wasMoving = isMoving;
            pause(0.01);
    ...
    /**
     * Sends a request for the finch to move forward or backward a given
     * distance at a given speed. Direction should be specified as "F" or "B".
    ...
    public void setMotors(double leftSpeed, double rightSpeed) {
        ...
        String[] urlArgs = {"out", "wheels", deviceInstance,
                            Double.toString(leftSpeed),
                            Double.toString(rightSpeed)};
        String url = getUrl(urlArgs);
        httpRequestOut(url);
    }
     ...
     * @param port - led to set. 1 is beak. 2-5 are tail. 6 sets entire tail.
     *     (Range: 1 to 6)
     ...
     */
    private void setTriLED(int port, int redIntensity, int greenIntensity,
                           int blueIntensity) {
        ...
        String[] urlArgs = {"out",
                            "triled",
                            portString,
                            Integer.toString(redIntensity),
                            Integer.toString(greenIntensity),
                            Integer.toString(blueIntensity),
                            deviceInstance};
        String url = getUrl(urlArgs);
        httpRequestOut(url);
    }
    ...
    public void setTail(String ledNum, int red, int green, int blue) {
        if (!ledNum.equals("all") && !ledNum.equals("All") &&
            !ledNum.equals("ALL")) {
            System.out.println(
                "Error: Please specify tail led number or 'all'");
            return;
        }
     ...
     * @return - sensor value returned by bluebird connector or -1 in the case
     *     of a problem.
     */
    ...
    private boolean getOrientationBoolean(String orientation) {
        String[] urlArgs = {"in", "finchOrientation", orientation,
                            deviceInstance};
        String url = getUrl(urlArgs);
    ...
    /**
     * getOrientation() provides information about the finch's current
     * orientation. This function overrides the function in the Robot class so
     * that results are in the finch reference frame.
     *
     * @return the orientation of the finch. (Range: Beak up, Beak down, Tilt
     *     left, Tilt right, Level, Upside down)
     */
