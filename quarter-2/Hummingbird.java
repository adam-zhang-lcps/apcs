/**
 * This class extends the Microbit class to incorporate functions to control the
 * inputs and outputs of the Hummingbird Bit. It includes methods to set the
 * values of motors and LEDs, as well as methods to read the values of the
 * sensors.
 *
...
public class Hummingbird extends Robot {
    ...
    /**
     * General constructor for the library. Set the device to be "A", "B", or
     * "C".
     *
     * @param device The letter corresponding to the Hummingbird device, which
     *     much be "A", "B", or "C".
     * The letter that identifies the Hummingbird device is assigned by the
     * BlueBird Connector.
     *      */
    ...
    /**
     * This function tries to read sensor 4 (the Hummingbird battery) to
     * determine whether or not the device is a Hummingbird.
     */
    private boolean isHummingbird() {
        StringBuilder newURL = new StringBuilder(baseUrl);
        String testURL =
            (newURL.append("in/isHummingbird/static/").append(deviceInstance))
                .toString();
        ...
        try {
               ...
               if (stringResponse.equals("255")) {
                   System.out.println("Error: Device "+deviceInstance+" is not a
        Hummingbird"); return false;
               }
               else {
                   return true;
               }
                } catch (IOException e) {
               System.out.println("Error: Device " + deviceInstance + " is not
        connected"); return false;
           }
        */
    }
    /* This function checks whether a port is within the given bounds. It
       returns a boolean value that is either true or false and prints an error
       if necessary. */
    protected boolean isPortValid(int port, int portMax) {
        if ((port < 1) || (port > portMax)) {
            System.out.println(
                "Error: Please choose a port value between 1 and " + portMax);
            return false;
        } else
            return true;
    }
    ...
    /**
     * setPositionServo sets the positionServo at a given port to a specific
     * angle. The function shows a warning dialog if the inputs are not in the
     * specified range.
     *
     * @param port     The port that the position servo is attached to. (Range:
     *     1-4)
    ...
    /**
     * setRotationServo sets the rotationServo at a given port to a specific
     * speed. The function shows a warning dialog if the inputs are not in the
     * specified range.
     *
     * @param port  The port that the rotation servo is attached to. (Range:
     *     1-4)
     ...
     *
     * @param port           The port that the LED is attached to. (Range: 1-4)
     * @param redIntensity   The intensity of red light of the triLED. (Range:
     *     0-100)
     * @param greenIntensity The intensity of green light of the triLED. (Range:
     *     0-100)
     * @param blueIntensity  The intensity of blue light of the triLED. (Range:
     *     0-100)
     */
    public void setTriLED(int port, int redIntensity, int greenIntensity,
                          int blueIntensity) {
    ...
    /**
     * getLight returns light sensor value at a given port after processing the
     * raw sensor value retrieved. The function shows a warning dialog if the
     * inputs are not in the specified range.
     *
    ...
    /**
     * getSound returns sound sensor value at a given port after processing the
     * raw sensor value retrieved. The function shows a warning dialog if the
     * inputs are not in the specified range.
     *
    ...
    /**
     * getDistance returns distance sensor value at a given port after
     * processing the raw sensor value retrieved. The function shows a warning
     * dialog if the inputs are not in the specified range.
     *
     * @param port The port that the distance sensor is attached to. (Range:
     *     1-3)
     */
    ...
    /**
     * getDial returns dial value at a given port after processing the raw
     * sensor value retrieved. The function shows a warning dialog if the inputs
