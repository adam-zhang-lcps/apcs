...
// Chris Schroeder: Modified to update the Swing UI on demand instead of using
// two threads and timers.
...
public class InsertionSortPanel extends JPanel implements ISortPanel {
    ...
    /**
     * Implement the drawPanel method, hiding the Swing UI elements from the
     * sorting object
     *
    ...
    /**
     * Get the graphics area we are painting to. If the image area does not
     * exists, then create it first
     * @return Graphics area to pain to....
     */
    private Graphics getPanelGraphics() {
        // set up the buffer
        if (null == myImage) {
            myImage = new BufferedImage(screenSize, screenSize,
    ...
    /**
     * Draw the given array onto the panel. This redraws everything instead of
    ...
    private void drawArray(int anArray[]) {
        ...
        for (int i = 0; i < anArray.length; i++) {
            ...
            graphics.fillRect((screenSize / anArray.length) * i,
                              ...
                              (screenSize / anArray.length) / 2,
                              (((screenSize - 100) / scale) * anArray[i]));

            // draw number on rectangle
            graphics.drawString(
                anArray[i] + "", (screenSize / anArray.length) * i,
    ...
    /**
     * Generate a new array that contains random numbers is is properly
     * shuffled.
     * @return A newly minted array of integers in all kinds of fun random
     *     order.
     */
