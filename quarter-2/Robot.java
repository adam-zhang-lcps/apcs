...
/**
 * This is an abstract class that is inherited by Microbit.java,
 * Hummingbird.java and Finch.java. It includes methods to print on the
 * micro:bit LED array or set those LEDs individually. It also contains methods
 * to read the values of the micro:bit accelerometer and magnetometer.
 *
 * Mike Yuan and Bambi Breewer, BirdBrain Technologies LLC
 * November 2018
 */
abstract class Robot {
    // Variables used to make http request to control the micro:bit (and
    // Hummingbird Bit)
    ...
    /* This function tests a connection by attempting to read whether or not the
     * micro:bit is shaking. Return true if the connection is good and false
     * otherwise.
     */
    ...
    /* This function checks whether an input parameter is within the given
       bounds. If not, it prints a warning and returns a value of the input
       parameter that is within the required range. Otherwise, it just returns
       the initial value. */
    protected int clampParameterToBounds(int parameter, int inputMin,
                                         int inputMax) {
        if ((parameter < inputMin) || (parameter > inputMax)) {
            System.out.println("Warning: Please choose a parameter between " +
                               inputMin + " and " + inputMax);
    ...
    /* This function checks whether an input parameter is within the given
       ...
       parameter that is within the required range. Otherwise, it just returns
       the initial value. */
    protected double clampParameterToBounds(double parameter, double inputMin,
                                            double inputMax) {
        if ((parameter < inputMin) || (parameter > inputMax)) {
            System.out.println("Warning: Please choose a parameter between " +
                               inputMin + " and " + inputMax);
    ...
    protected String sendHttpRequest(String URLRequest) {
        ...
        try {
            ...
            if (responseString.equals("Not Connected")) {
                System.out.println("Error: Device " + deviceInstance +
                                   " is not connected");
            }
            disconnect();
        }
        // If too many requests get sent too quickly, macOS gets overwhelmed and
        // starts to insert pauses.
    ...
    /* This function sends http requests that set outputs (lights, motors,
     * buzzer, etc.) on the micro:bit and Hummingbird. */
    protected void httpRequestOut(String URLRequest) {
        String response = sendHttpRequest(URLRequest);
    }

    /* This function sends http requests that return a double response from a
     * sensor. */
    ...
    /* This function sends http requests that return a boolean response from a
     * sensor. */
    ...
    public void print(String message) {
        ...
        // Warn the user if there are any special characters. Note that we don't
        // use isCharacterOrDigit() because we can only display English
        // characters
        char letter;
    ...
    /**
     * setDisplay lets the LED Array display a pattern based on an array of 1s
     * and 0s.
     *
     * @param ledValues The list of integers that the function takes in to set
     *     the LED Array.
    ...
    public void setDisplay(int[] ledValues) {
        ...
        if (ledLen != 25) {
            System.out.println(
                "Error: setDisplay() requires a int array of length 25");
            return;
        }

        /* For the http request, we need to convert the 0s and 1s to boolean
         * values. We can do this while also ensuring that the user only used 0
         * and 1.
         */
        for (int i = 0; i < ledLen; i++) {
            if (ledValues[i] == 1)
        ...
        resultUrl = resultUrl.append("out/").append("symbol/").append(
            deviceInstance.toString() + "/");

        for (int i = 0; i < ledLen; i++) {
            resultUrl =
                resultUrl.append(String.valueOf(displayStatus[i]) + "/");
        }
    ...
    public void setPoint(int row, int column, int value) {
        ...
        /* For the http request, we need to convert the 0 or 1 to a boolean. We
         * can do this warning if the user didn't use 0 or 1 for the value
         */
        if (value == 1)
     ...
     * @param beats - duration in beats (Range: 0 to 16); each beat is one
     *     second
     */
    public void playNote(int note, double beats) {
        ...
        String[] urlArgs = {"out", "playnote", Integer.toString(note),
                            Integer.toString((int)beats), deviceInstance};
        String url = getUrl(urlArgs);
        httpRequestOut(url);
    }

    /**
     * getAccelerationInDirs returns acceleration value in a specified
     * direction.
     *
    ...
    /**
     * getMagnetometerValInDirs returns magnetometer value in a specified
     * direction.
     *
     * @param dir The direction of which the magnetometer value will be
     *     returned.
     */
    ...
    /**
     * getMagnetometer returns magnetometer values in 3 directions (X,Y,Z) in
     * microT.
     *
     ...
     * The function shows a warning dialog if the inputs are not in the
     * specified range.
     *
     * @param button the button that will be checked whether is it pressed or
     *     not. (Range: "A", "B")
    ...
    /**
     * getSound() returns the current sound level from the micro:bit sound
     * sensor
     * @return sound level
     */
    public int getSound() {
    ...
    /**
     * getTemperature() returns the current temperature in degrees Celcius from
    ...
    /**
     * getOrientationBoolean checks whether the device currently being held to a
     * specific orientation or shaken.
     *
     * @param orientation The orientation that will be checked.
     * @return "true" if the device is held to the orientation and false
     *     otherwise.
     */
     ...
     * */
    public boolean isShaking() { return getOrientationBoolean(SHAKE); }
    /**
     * getOrientation() provides information about the device's current
     * orientation.
     *
     * @return the orientation of the device. (Range: Screen up, Screen down,
    ...
    public void stopAll() {
        ...
        pause(0.1); // Hack to give stopAll() time to act before the end of a
                    // program

        // Build http request to turn off all the outputs
        StringBuilder resultUrl = new StringBuilder(baseUrl);
        String stopUrl =
            (resultUrl.append("out/").append("stopall/").append(deviceInstance))
                .toString();
